...
public class Manage_Resolution extends JFrame {
	...
	public Manage_Resolution() {
		...
		titledBorder = new TitledBorder(UIManager.getBorder("TitledBorder.border"), "共"+ResolutionDao.getInstance().QueryCount()+"项", TitledBorder.LEADING, TitledBorder.TOP, null, Color.RED);
		...
		listTable.addMouseListener(new MouseAdapter() {   //用户名表格点击事件
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				CURRENTID = ResolutionDao.getInstance().QueryIDByName(bjsName);//获得当前选中分辨率的id
		...
		saveButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(bjsName.equals("")) //分辨率名为空
					JOptionPane.showMessageDialog(null,"分辨率名为空","提示",JOptionPane.INFORMATION_MESSAGE);
				else {  //分辨率名不为空
					if(CURRENTID !=0 ){  //当前选中了一项
						//修改之后的分辨率已经存在
						if(!ResolutionDao.getInstance().QueryNameByID(CURRENTID).equals(bjsName) && ResolutionDao.getInstance().IsExist(bjsName))
							JOptionPane.showMessageDialog(null,"该分辨率名已经存在","提示",JOptionPane.INFORMATION_MESSAGE);
						else//修改之后的分辨率不存在					
							ResolutionDao.getInstance().Update(bjsName,CURRENTID);
					}
					else if(CURRENTID ==0 ){  //添加分辨率
						if(ResolutionDao.getInstance().IsExist(bjsName))
							JOptionPane.showMessageDialog(null,"该分辨率名已经存在","提示",JOptionPane.INFORMATION_MESSAGE);
						else {
							ResolutionDao.getInstance().Add(bjsName);
							CURRENTID = ResolutionDao.getInstance().QueryIDByName(bjsName);//更新当前id指针为新建用户的id
							deleteButton.setEnabled(true);
							resetButton.setEnabled(true);
							titledBorder1.setTitle("修改");
							settingPanel.repaint();
		...
		deleteButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int selection = JOptionPane.showConfirmDialog(null,"确认删除"+ResolutionDao.getInstance().QueryNameByID(CURRENTID)+"？","删除",JOptionPane.OK_CANCEL_OPTION,JOptionPane.QUESTION_MESSAGE);
				if( selection == JOptionPane.OK_OPTION ){
					String bjsName = nameTextField.getText();
					ResolutionDao.getInstance().Delete(bjsName);
    ...
    public Object[][] queryData(){
        ...
        for(int i=0;i<list.size();i++){
            for(int j=0;j<head.length;j++){
                data[i][0]=list.get(i).getResolutionName();
            }
        }
        return data;
    }
	...
	public void RefreshListPanel() {
    	...
    	listTable.setModel(tableModel);
    	scrollPane.setViewportView(listTable);
    	titledBorder.setTitle("共"+ResolutionDao.getInstance().QueryCount()+"项");
		listPanel.repaint();
		listPanel.validate();
		listPanel.repaint();
	}
	...
	public void SetSelected(){
		String queryName = ResolutionDao.getInstance().QueryNameByID(CURRENTID);
