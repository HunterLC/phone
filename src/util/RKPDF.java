...
import model.RK;
public class RKPDF {
	private static List<RK> rkDate;

	public RKPDF(List<RK> date) {
		this.rkDate = date;
	}

	public static void createPDF(String path,String time) throws DocumentException, IOException {
		...
		Phrase phrase1 = new Phrase(time+"手机入库信息一览表", font);
		Phrase phrase2 = new Phrase("报表生成日期：" + df.format(new Date()), font1);
		ColumnText.showTextAligned(canvas, Element.ALIGN_CENTER, phrase1, 330, 780, 0);
		...
		pdfCell.setPhrase(paragraph);
		table.addCell(pdfCell);
		paragraph = new Paragraph("入库时间", fontBold);
		pdfCell.setPhrase(paragraph);
		table.addCell(pdfCell);
		paragraph = new Paragraph("入库数量", fontBold);
		pdfCell.setPhrase(paragraph);
		table.addCell(pdfCell);
		paragraph = new Paragraph("入库次数", fontBold);
		pdfCell.setPhrase(paragraph);
		table.addCell(pdfCell);
		...
		for (int i = 0; i < rkDate.size(); i++) {
			for (int j = 0; j < 8; j++) {
				...
				Paragraph paragraph1 = new Paragraph(String.valueOf(rkDate.get(i).get(j)), font1);
				pdfCell1.setPhrase(paragraph1);
				table.addCell(pdfCell1);
			}
			rkTotal += rkDate.get(i).getTotalNumber();//计算总库存
			totalMoney += rkDate.get(i).getTotalNumber()*rkDate.get(i).getPrice();//计算最后的总价
		}
		...
		pdfCell1.setHorizontalAlignment(Element.ALIGN_RIGHT);
		paragraph1 = new Paragraph(new DecimalFormat("###,###,###.##").format(rkTotal), font1);
		pdfCell1.setPhrase(paragraph1);
		table.addCell(pdfCell1);
